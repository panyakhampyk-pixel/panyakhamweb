...
const Teachers = () => {
    ...
    const groupedTeachers = teachers.reduce((acc, current) => {
        ...
        return acc;
    }, {} as Record<number, { name: string; members: TeacherMember[] }>);
    ...
    return (
        <div className="min-h-screen bg-[#FDFCFB]">
            ...
            {/* Hero Header */}
            <div className="pt-32 pb-20 bg-amber-600 relative overflow-hidden">
                <div className="absolute inset-0 opacity-10">
                    <div className="absolute top-0 left-0 w-96 h-96 bg-white rounded-full -translate-x-1/2 -translate-y-1/2 blur-3xl"></div>
                    <div className="absolute bottom-0 right-0 w-96 h-96 bg-amber-400 rounded-full translate-x-1/2 translate-y-1/2 blur-3xl"></div>
                </div>
                ...
                <div className="container mx-auto px-4 relative z-10 text-center">
                    ...
                    <h1 className="text-4xl md:text-6xl font-black text-white mb-6 uppercase tracking-tight">
                        ทำเนียบ <span className="text-amber-200 italic">อาจารย์</span>
                    </h1>
                    <p className="text-amber-50/80 max-w-2xl mx-auto leading-relaxed text-lg font-medium">
                        คณะอาจารย์ผู้ทรงคุณวุฒิที่ร่วมถ่ายทอดความรู้และทักษะวิชาชีพ
                        เพื่อสร้างอนาคตที่ยั่งยืนให้กับนักเรียนทุนทุกคน
                    </p>
                </div>
            </div>
            ...
            <div className="bg-white border-b border-border">
                <div className="container mx-auto px-4 py-4 flex items-center gap-2 text-sm text-muted-foreground font-medium">
                    ...
                    <span className="text-foreground font-bold">ทำเนียบอาจารย์</span>
                </div>
            </div>
            ...
            <main className="py-24 container mx-auto px-4 min-h-[60vh]">
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-20 gap-4">
                        ...
                        <p className="font-bold text-muted-foreground animate-pulse">กำลังโหลดข้อมูลอาจารย์...</p>
                    </div>
                ) : teachers.length === 0 ? (
                    <div className="text-center py-20 bg-white rounded-[3rem] border border-dashed border-border">
                        ...
                        <h3 className="text-xl font-bold text-muted-foreground">ไม่พบข้อมูลอาจารย์</h3>
                    ...
                    <div className="space-y-32">
                        {Object.keys(groupedTeachers).sort((a, b) => parseInt(a) - parseInt(b)).map((level) => {
                            const group = groupedTeachers[parseInt(level)];

                            return (
                                <div key={level} className="space-y-16">
                                    {/* Group Header */}
                                    <div className="relative text-center">
                                        <h2 className="relative z-10 font-black uppercase tracking-[0.2em] inline-block px-10 py-3 rounded-full bg-white border border-border shadow-sm text-lg text-amber-600">
                                            {group.name}
                                        </h2>
                                    ...
                                    <div className={`flex flex-wrap justify-center gap-x-12 gap-y-20`}>
                                        {group.members.map((person) => (
                                            <div
                                                key={person.id}
                                                className="flex flex-col items-center text-center group max-w-[240px]"
                                            >
                                                {/* Circular Image Container */}
                                                <div className="relative mb-8 transition-all duration-500 group-hover:scale-105 w-44 h-44">
                                                    {/* Decorative Ring */}
                                                    <div className="absolute inset-0 rounded-full border-2 border-amber-100 scale-110 group-hover:scale-115 transition-transform duration-500"></div>
                                                    <div className="absolute inset-0 rounded-full border border-amber-500 scale-125 opacity-0 group-hover:opacity-20 group-hover:scale-135 transition-all duration-700"></div>
                                                ...
                                                <div className="space-y-2 px-4">
                                                    <h4 className="font-black text-slate-900 leading-tight text-xl">
                                                        {person.name}
                                                    </h4>
                                                    <p className="font-bold text-amber-600 uppercase tracking-widest text-xs">
                                                        {person.position}
                                                    </p>
                                                </div>
                                            </div>
